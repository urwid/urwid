...
awk '/Authors/,/Contributors/' README.rst | grep -E "^\`\S+ <//" | cut -d\` -f 2 |
    cut -d' ' -f 1 > authors.${PREV_RELEASE}.txt
...
function install_contributors() {
    # the exclude_contribution_types branch is submitted as upstream PR:
    # https://github.com/pydanny/contributors/pull/322
    # it lets us track committers and PR/issue creators separately
    pip install -U git+https://github.com/tonycpsu/contributors@exclude_contribution_types
}

function generate_contributors() {

    contributors -f rst -x issue -x pr --since ${PREV_RELEASE_DATE} \
        -o authors.$RELEASE.rst urwid/urwid

    contributors -f rst -x commit --since ${PREV_RELEASE_DATE} \
...
grep -E "^\." contributors.$RELEASE.rst \
    | cut -d@ -f 2 | cut -d\` -f 1 \
    | grep -Ev "^(invalid-email-address|ghost)$" \
    | grep -vf authors.$RELEASE.txt > contributors.$RELEASE.txt

cat authors.$PREV_RELEASE.txt authors.$RELEASE.txt \
    > authors.$RELEASE.combined.txt

cat contributors.$PREV_RELEASE.txt contributors.$RELEASE.txt \
    | grep -vf authors.$RELEASE.combined.txt \
    > contributors.$RELEASE.combined.txt

(
    for id in $(sort -f authors.$RELEASE.combined.txt | uniq); do
        if [[ " ${MAINTAINERS[@]} " =~ " ${id} " ]]; then
            continue
        fi
        echo "\`${id} <//github.com/${id}>\`_,"
    done
) | sed '$ s/.$//' > authors.${RELEASE}.rst
...
rm -f authors.$PREV_RELEASE.* authors.$RELEASE.* \
    contributors.$PREV_RELEASE.* contributors.$RELEASE.* \
    changes.$RELEASE.txt
