...
def set_encoding( encoding ):
    ...
    try:
        if encoding:
            u"".encode(encoding, errors='replace')
...
def apply_target_encoding( s ):
    ...
    SO = escape.SO.encode('ascii', errors='replace')
    SI = escape.SI.encode('ascii', errors='replace')
    ...
    for sn in sis[1:]:
        ...
        if len(sl) == 1:
            ...
            sout.append(sin)
            rle_append_modify(cout, (escape.DEC_TAG.encode('ascii', errors='replace'), len(sin)))
            continue
        sin, son = sl
